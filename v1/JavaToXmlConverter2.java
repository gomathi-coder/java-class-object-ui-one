...
public class JavaToXmlConverter2 {
    ...
    private static String convertJavaToXml(String javaCode) {
        // Remove single-line comments but keep multi-line comments for method bodies
        String codeWithoutSingleLineComments = javaCode.replaceAll("//.*", "");
        
        // Extract package declaration
        String packageName = extractPackage(codeWithoutSingleLineComments);
        
        // Extract imports
        List<String> imports = extractImports(codeWithoutSingleLineComments);
        
        // Extract class information
        ClassInfo classInfo = extractClassInfo(codeWithoutSingleLineComments);
        ...
        if (!classInfo.methods.isEmpty()) {
            ...
            for (MethodInfo method : classInfo.methods) {
                ...
                // Add method body/definition
                if (method.body != null && !method.body.trim().isEmpty()) {
                    xml.append("        <definition><![CDATA[").append(method.body).append("]]></definition>\n");
                }
    ...
    private static ClassInfo extractClassInfo(String code) {
        ...
        while (methodMatcher.find()) {
            ...
            // Process method body
            if (methodMatcher.group(6) != null) {
                method.body = methodMatcher.group(6).trim();
            }
